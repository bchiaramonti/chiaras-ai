...
| 2026-04-17 | Q2 docs sync       | Skills: added `when_to_use` field, clarified combined 1,536-char cap, added `xhigh` effort tier, removed incorrect 250-char claim, added skill content lifecycle (re-attach with 5k per skill / 25k total after compaction), added multi-line `` ```! `` shell block syntax, `disableSkillShellExecution` setting. Agents: added `color` field, `auto` permission mode, parent-mode precedence rules, plugin subagent restrictions (`hooks`/`mcpServers`/`permissionMode` ignored), agent-teams subagent-definition semantics (`skills`/`mcpServers` skipped), removed incorrect `Bash` built-in subagent from table. Plugins: added `monitors/monitors.json` + `monitors` field, `bin/` directory on Bash PATH, `dependencies` field with semver, `subagentStatusLine` settings key, `claude plugin list [--json] [--available]` CLI, orphaned cache 7-day cleanup note, multi-`--plugin-dir` support. Marketplace: added `knowledge-work-plugins` to reserved names, `@ref` / `#ref` add shortcuts, `claude plugin marketplace` CLI subcommands incl. `--sparse`, `CLAUDE_CODE_PLUGIN_KEEP_MARKETPLACE_ON_FAILURE`, `CLAUDE_CODE_PLUGIN_CACHE_DIR` build-time seeding, seed precedence rules. Hooks: added `PermissionDenied` event with `retry`, `asyncRewake` option, `defer` permission decision with precedence (`deny > defer > ask > allow`), `updatedMCPToolOutput`, `updatedPermissions` types (addRules/replaceRules/removeRules/setMode/addDirectories/removeDirectories), `allowManagedHooksOnly` managed setting, full headless deferred-tool-call flow. |
