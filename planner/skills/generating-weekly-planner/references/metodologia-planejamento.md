...
**Metodologias-base aplicadas** (pesquisadas e validadas):
- **Michael Hyatt · Full Focus Planner** → Weekly Preview ritual (Retrospectiva → Big 3 → Schedule → Self-care) + Weekly Big 3 derivados do Quarterly Big 3
- **Cal Newport · Time Blocking / Deep Work** → cada hora tem uma funcao, deep work blocks nao-negociaveis, shutdown ritual
- **4DX (McChesney) · Lead Measures** → Criterio de vitoria com indicadores lead (o que eu controlo) + lag (o que prova)
- **Kahneman/Klein · Pre-Mortem** → imaginar fracasso antes de comecar, mitigar preventivamente
- **Newport · Shutdown Reverso** → Preflight antes da semana em vez de revisao so depois
...
**O que e:** a **aposta** da semana — o que voce esta comprando com os 5 dias, argumentada em prosa editorial. E a uniao entre a Retrospectiva S-1 (o que destravou/travou) e as Metas Q2 (onde voce precisa avancar).

**Analogia com o daily:** se o Lide responde *"qual a tese de HOJE?"*, a Tese responde *"qual a aposta da SEMANA?"* — mesma ideia, horizonte diferente.
...
- **Referencia a retrospectiva** (o que foi destravado ou aprendido)
- **Referencia a Q2** (qual objetivo avanca)
- **Implicacao/destrave** (o que abre quando essa semana fechar)

Se a Tese e so "vou fazer X, Y e Z esta semana" = lista, nao tese. Reescreva.
...
**Inspiracao:** 4DX Lead Measures (McChesney) + Hyatt Self-Care (5 areas: sleep, eat, move, connect, relax).
...
- Ruim: "Avancar no m7-controle"
- Bom: "WBR Invest aprovado pela diretoria"
- Bom: "3 treinos longos + sono medio ≥ 7h"

### Anti-padroes

| Ruim | Melhor |
|---|---|
| "Ser produtivo" | "Weekly planner skill publicada" |
| "Cuidar do corpo" | "3 treinos + sono medio ≥ 7h" |
| 6 checks (demais) | Cortar para 4 — mais que isso vira lista |
| 4 checks de trabalho, zero de corpo | Trocar 1 por check de corpo/recovery (balance obrigatorio) |

### Teste de sanidade

Pedir mentalmente: "se na sexta eu riscar os 4, eu estou orgulhoso da semana?" Se a resposta e "mais ou menos", os criterios estao fracos. Reescrever.

## Regra 3 · Orquestra dos 5 dias

**O que e:** o **coracao do weekly**. Como os 5 dias se encaixam para cumprir a Tese. Nao e agenda — e orquestracao. Cada dia recebe um **papel proprio** na narrativa.

**Inspiracao:** Cal Newport Time Blocking ("every minute has a job") + Shape Up cycles aplicados em escala microscopica (5 dias = 5 micro-ciclos).
...
- Dia do deep work protegido → "alta · deep"
- Dia de apresentacao / evento externo → "alta"
- Dia apos noite mal dormida (se souber) → "baixa" e remapear

**Entrega de cada dia.** Toda Orquestra deve ter uma "linha de entrega" horizontal: se alinhar visualmente, a semana e clara; se desalinhar (alguns dias sem entrega clara), tem dia de "entupimento".

### Anti-padroes

| Ruim | Melhor |
|---|---|
| 5 dias com meetings das 9h as 18h | Proteger pelo menos 1 dia para deep work |
| MIT #1 espalhado em 30min por dia | Bloqueo contiguo de 4h+ em 1 dia |
| Tema do dia = lista de tarefas | Tema = 1 linha narrativa do papel do dia |
...
Verificar que os 3 cobrem pelo menos **2 das 3 dimensoes**:
- Trabalho · foco estrategico
- Corpo · treino / recovery / saude
- Familia/Pessoal (menos comum em weekly, mais em daily)

Se 3 sao todos de trabalho, pode ser sinal de que a semana esta desbalanceada. Recalibrar **se e vespera de compromisso familiar ou ciclo de treino all-in**.
...
Dentro do bloco, ordenar:
1. Atrasados (topo, alert)
2. Ordem cronologica dos dias (seg → sex)

### Metadata obrigatoria por prazo

- **Dia ancora** (seg 13 · ter 14 · qua 15 · qui 16 · sex 17)
- **Titulo da entrega**
- **SLA especifico** (se tiver hora, ex: "10h")
- **Status** (atrasado / hoje / futuro)
...
**O que e:** **antes** da semana comecar, imaginar o que pode dar errado, e escrever a **mitigacao pronta**. Inspirado em Kahneman/Klein (Pre-mortem).

**Principio:** riscos sem mitigacao sao so ansiedade documentada. Cada risco deve ter uma acao concreta preventiva.

### Filtro

Derivar 3-4 riscos dos seguintes sinais:
...
mitigacao: <acao concreta preventiva, curta, acionavel>
```
...
- **Qua 15 protegida pode ser invadida**
  mitigacao: bloquear agenda seg; avisar time que qua = maker day

### Regras de mitigacao

**Acionavel, concreta, com verbo** — nao "organizar melhor", mas "bloquear agenda seg".

**Quando possivel, ancorar a dia** — "escalar com Fernando na **ter**", "dormir 22h30 na **qui**".

**Multiplas mitigacoes por risco** — separar por `;` se forem acoes independentes.

### Anti-padroes

| Ruim | Melhor |
|---|---|
| "Risco: tudo pode dar errado" | Risco concreto e especifico |
| "Mitigacao: me organizar melhor" | "Bloquear agenda seg" |
| Mitigacao sem dia | Ancorar ao dia especifico quando possivel |
| 8 riscos | Cortar para 3-4 — se ha 8, nao ha priorizacao |

## Regra 7 · Preflight

**O que e:** **4 perguntas editoriais** que o usuario responde em italic curto **antes** de comecar a semana. Inspirado no shutdown ritual de Newport, mas **invertido** — feito na vespera da semana em vez de depois.

**Funcao:** forcar clareza estrategica antes do ruido comecar. E o ultimo filtro antes de entrar na semana.

### As 4 perguntas (obrigatorias, nesta ordem)

1. **O que define vitoria nesta semana?**
   Resposta: 1 frase condensando o Criterio de vitoria (Regra 2) na essencia.
   Exemplo: *"Cards aprovados pela diretoria e publicados em producao na sex — o resto e consequencia."*

2. **Onde esta o deep work?**
   Resposta: dias + horas bloqueadas para producao profunda.
   Exemplo: *"Qua 15 blindada (7h total) · manhas de seg e ter (2h cada) · nao dilua."*

3. **Onde vou dizer nao?**
   Resposta: convites/distracoes que JA estao no radar para recusar.
   Exemplo: *"Qualquer convite para qua 15 · 1:1s novos com assessores (remarcar p/ S17)."*

4. **Qual o maior risco?**
   Resposta: 1 risco com mitigacao imediata.
   Exemplo: *"Chegar na sex sem ensaiar a apresentacao — forcar ensaio na qui 17h, sem excecao."*

### Regras de redacao

**Respostas em italic + sentence case + ponto final** (editorial).

**Concisas** — 1-2 linhas cada. Preflight nao e plano de acao detalhado, e filtro estrategico.

**Concretas** — nome de projeto, hora, dia. Nao "fazer deep work" mas "qua 15 · 7h".

### Anti-padroes

| Ruim | Melhor |
|---|---|
| Respostas em prosa longa | 1-2 linhas concisas em italic |
| "O que define vitoria?" = "fazer tudo" | "Cards aprovados na sex" |
